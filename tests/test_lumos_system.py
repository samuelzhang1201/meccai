...
def test_workflow_task_analysis():
    ...
    for task in tableau_tasks:
        assert (
            "tableau" in task.lower()
            or "dashboard" in task.lower()
            or "visualization" in task.lower()
        )

    # Test dbt-related tasks
    dbt_tasks = ["run dbt models", "transform the data", "build data models"]

    for task in dbt_tasks:
        assert (
            "dbt" in task.lower()
            or "transform" in task.lower()
            or "model" in task.lower()
        )
...
def test_system_configuration():
    ...
    assert (
        settings.tableau.server_url == "https://prod-apsoutheast-a.online.tableau.com"
    )
