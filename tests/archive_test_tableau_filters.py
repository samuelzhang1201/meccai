...
async def test_tableau_filtering():
    ...
    try:
        ...
        if updated_history and len(updated_history) > 1:
            ...
            found_keywords = [
                kw for kw in filter_keywords if kw.lower() in response.lower()
            ]
            ...
            if len(response) < 2000:  # Shorter response indicates filtering worked
                print(
                    f"   ✅ Response is concise ({len(response)} chars) - filtering likely worked"
                )
                ...
                print(
                    f"   ⚠️  Response is long ({len(response)} chars) - filtering may not have worked"
                )
...
def main():
    ...
    print(
        f"   Workbook filtering: {'✅ PASS' if workbook_success else '⚠️  NEEDS REVIEW'}"
    )
