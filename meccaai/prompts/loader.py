...
def parse_prompt_metadata(prompt_content: str) -> Dict[str, Any]:
    ...
    instructions_match = re.search(r'<instructions>(.*?)</instructions>', prompt_content, re.DOTALL)
    if instructions_match:
        metadata['instructions'] = instructions_match.group(1).strip()
    
    # Extract parameters
    parameters_match = re.search(r'<parameters>(.*?)</parameters>', prompt_content, re.DOTALL)
    if parameters_match:
