...
class AtlassianAuthManager:
    ...
    def __init__(self):
        ...
        self.auth_method = getattr(
            settings, "atlassian_auth_method", "sso"
        )  # 'sso', 'token', 'oauth'
    ...
    def get_auth_headers(self) -> Dict[str, str]:
        ...
        if self.auth_method == "sso" and self.session_token:
            ...
            logger.warning(
                "No Atlassian authentication configured. API calls may fail."
            )
            return base_headers
...
async def get_jira_issue(
    ...
    if not auth_manager.jira_base_url:
        return ToolResult(
            success=False, result={"error": "Jira base URL not configured"}
        )
    ...
    try:
        ...
        return ToolResult(
            success=False, result={"error": f"Failed to get issue: {str(e)}"}
        )
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            response = await client.put(
                url, json=payload, headers=auth_manager.get_auth_headers(), timeout=30.0
            )
            response.raise_for_status()
            ...
            if status is not None:
                ...
                transitions_response = await client.get(
                    transition_url, headers=auth_manager.get_auth_headers()
                )
        ...
        return ToolResult(
            success=False, result={"error": f"Failed to update issue: {str(e)}"}
        )
...
async def create_jira_issue(
    ...
    try:
        ...
        return ToolResult(
            success=False, result={"error": f"Failed to create issue: {str(e)}"}
        )
...
async def search_jira_issues_using_jql(
    ...
    try:
        ...
        return ToolResult(
            success=False, result={"error": f"Failed to search issues: {str(e)}"}
        )


@tool("add_comment_to_jira_issue")
async def add_comment_to_jira_issue(
    issue_key: str, comment: str, visibility: Optional[str] = None
) -> ToolResult:
    ...
    try:
        ...
        return ToolResult(
            success=False, result={"error": f"Failed to add comment: {str(e)}"}
        )
...
async def get_visible_jira_projects(
    ...
    try:
        ...
        return ToolResult(
            success=False, result={"error": f"Failed to get projects: {str(e)}"}
        )


@tool("get_jira_project_issue_types_metadata")
async def get_jira_project_issue_types_metadata(project_key: str) -> ToolResult:
    ...
    try:
        ...
        return ToolResult(
            success=False, result={"error": f"Failed to get project metadata: {str(e)}"}
        )
...
async def get_confluence_page(
    page_id: str, expand: str = "body.storage,version,space,ancestors"
) -> ToolResult:
    ...
    if not auth_manager.confluence_base_url:
        return ToolResult(
            success=False, result={"error": "Confluence base URL not configured"}
        )
    ...
    try:
        ...
        return ToolResult(
            success=False, result={"error": f"Failed to get page: {str(e)}"}
        )
...
async def create_confluence_page(
    ...
    try:
        ...
        return ToolResult(
            success=False, result={"error": f"Failed to create page: {str(e)}"}
        )
...
async def update_confluence_page(
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            # Build update payload
            payload = {"version": {"number": version_number + 1}}
        ...
        return ToolResult(
            success=False, result={"error": f"Failed to update page: {str(e)}"}
        )
